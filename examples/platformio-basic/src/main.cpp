...
void setup() {
  ...
  Log.notice(
      F(ARDUINO_LOG_CR "******************************************" ARDUINO_LOG_CR)); // Info string
                                                              // with Newline
  Log.notice(
      "***          Logging example                " ARDUINO_LOG_CR); // Info string in
                                                          // flash memory
  Log.notice(F("******************* "));
  Log.notice("*********************** " ARDUINO_LOG_CR); // two info strings without newline
}
...
void loop() {
  ...
  Log.notice("Log as Info with integer values : %d, %d" ARDUINO_LOG_CR, intValue1,
             intValue2);
  Log.notice(F("Log as Info with hex values     : %x, %X" ARDUINO_LOG_CR), intValue1,
             intValue1);
  Log.notice("Log as Info with hex values     : %x, %X" ARDUINO_LOG_CR, intValue2,
             intValue2);
  Log.notice(F("Log as Info with binary values  : %b, %B" ARDUINO_LOG_CR), intValue1,
             intValue1);
  Log.notice("Log as Info with binary values  : %b, %B" ARDUINO_LOG_CR, intValue2,
             intValue2);
  Log.notice(F("Log as Info with long values    : %l, %l" ARDUINO_LOG_CR), longValue1,
             longValue2);
  Log.notice("Log as Info with bool values    : %t, %T" ARDUINO_LOG_CR, boolValue1,
             boolValue2);
  ...
  Log.notice("Log as Info with Flash string value   : %S" ARDUINO_LOG_CR, flashCharArray2);
